...
internal class Program
{
    private static void Main(string[] args)
    {
...

        {
            ...
            void Greedy02()
            {
                var input = Console.ReadLine()!.Select(c => int.Parse(c.ToString())).ToList();

                //var output = 0;
                for (int i = 1; i < input.Count; i++)
                {
                    input[i] = Math.Max((input[i - 1] + input[i]), (input[i - 1] * input[i]));
                }
            ...
            void Greedy03()
            {
                var inputArray = new int[int.Parse(Console.ReadLine()!)];
                var inputScared = Console.ReadLine()!.Split(" ").Select(int.Parse).ToArray();
                for (int i = 0; i < inputScared.Length; i++)
                {
                    inputArray[i] = inputScared[i];
                }
                Array.Sort(inputArray);
            ...
            void Implementation01()
            {
                // 2차원 배열의 한변의 크기
                var arraySize = int.Parse(Console.ReadLine()!);
                var arraySpace = new int[arraySize, arraySize];

                var position = new Vector2(1, 1);
                var arrayPath = Console.ReadLine()!.Split(" ");
